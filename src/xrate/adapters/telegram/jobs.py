...
async def daily_summary_job(context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    Send daily summary report to admin at scheduled time (8:00 AM local time).
...
async def daily_morning_post(context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    Daily morning post at 8:00 AM local time (excluding Thursday and Friday).
    
    Posts market data in Persian daily report format to admin user.
    """
    logger = logging.getLogger(__name__)
    ...
    if not admin_user_id:
        logger.warning("No admin user ID available - skipping daily morning post")
        return
    
    # Check if today is Thursday (3) or Friday (4) - skip if so
    from datetime import datetime, timezone
    from zoneinfo import ZoneInfo
    # Use local timezone (Asia/Tehran) for weekday check
    local_tz = ZoneInfo("Asia/Tehran")
    now_local = datetime.now(local_tz)
    weekday = now_local.weekday()  # Monday=0, Tuesday=1, Wednesday=2, Thursday=3, Friday=4, Saturday=5, Sunday=6
    ...
    try:
        ...
        try:
            ...
            logger.info("Daily morning post sent to admin at 8:00 AM local time")
        except Exception as e:
            logger.error("Failed to post to admin: %s", e)
            stats_tracker.record_error(str(e))
            return
